...
def get_whois_raw(domain, server="", previous=None, rfc3490=True, never_cut=False, with_server_list=False, server_list=None, timeout=DEFAULT_TIMEOUT):
	...
	if len(previous) == 0 and server == "":
		...
		if is_exception == False:
			target_server = get_root_server(domain, timeout)
	else:
	...
	for line in [x.strip() for x in response.splitlines()]:
		match = re.match("(refer|whois server|referral url|whois server|registrar whois|registrar whois server):\s*([^\s]+\.[^\s]+)", line, re.IGNORECASE)
...
def get_root_server(domain, timeout=DEFAULT_TIMEOUT):
...
def whois_request(domain, server, port=43, timeout=DEFAULT_TIMEOUT):
